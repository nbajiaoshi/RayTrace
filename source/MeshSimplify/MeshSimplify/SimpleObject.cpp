...
CSimpleObject::CSimpleObject(Point** points,int n){
    m_nVertices = n;
    m_pVertexList = new Vec3f[n];
    for (int i = 0;i < n;i++){
        m_pVertexList[i] = points[i]->p;
        points[i]->id = i;
    }
    m_nTriangles = 0;
    for (int i = 0;i < n;i++)
        m_nTriangles += points[i]->triangles.size();
    m_nTriangles /= 3;
    m_pTriangleList = new Array<int,3>[m_nTriangles];
    Triangle* tri;
    int m = 0;
    for (int i = 0;i < n;i++)
        for (int j = 0;j < points[i]->triangles.size();j++){
            tri = points[i]->triangles[j];
            if (i <= tri->a->id && i <= tri->b->id && i <= tri->c->id){
                m_pTriangleList[m][0] = tri->a->id;
                m_pTriangleList[m][1] = tri->b->id;
                m_pTriangleList[m][2] = tri->c->id;
                m++;
            }
        }
}
...
{
    for(unsigned i=0;i<vecTriangles.size();i++)
    {
        Array<int,3> & vIndices = vecTriangles[i];
        for(unsigned j=0;j<vIndices._len;j++)
